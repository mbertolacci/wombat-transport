...
fc="${FC:-/usr/bin/f95}"
src="${repo_root}/tools/gc_harness/vdiff_harness.F90"
trace_mod_src="${repo_root}/tools/gc_harness/vdiff_trace_mod.F90"
generated_vdiff="${out_dir}/vdiff_mod.harness.F90"
exe="${out_dir}/vdiff_harness"
...
"${repo_root}/tools/gc_harness/generate_vdiff_harness_source.py" \
  "${repo_root}/GCClassic/src/GEOS-Chem/GeosCore/vdiff_mod.F90" \
  "${generated_vdiff}" >/dev/null
...
"${fc}" "${common_compile_flags[@]}" -c "${trace_mod_src}" -o "${out_dir}/vdiff_trace_mod.o"
"${fc}" "${common_compile_flags[@]}" -cpp -c "${generated_vdiff}" -o "${out_dir}/vdiff_mod.harness.o"

"${fc}" \
  "${common_compile_flags[@]}" \
  "${src}" \
  "${out_dir}/vdiff_trace_mod.o" \
  "${out_dir}/vdiff_mod.harness.o" \
  "${link_flags[@]}" \
  -Wl,--start-group \
  "${gc_libs[@]}" \
  -Wl,--end-group \
