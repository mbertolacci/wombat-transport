...
The harness has grown from the initial PJC/TPCORE executable into a set of
GEOS-Chem-backed operator and handoff oracles for the configured global 2.0 x
2.5, 47-level transport-only target. It now covers PJC/PFIX, TPCORE, VDIFF,
convection, selected met diagnostics, HISTORY behavior, and main-loop tracing.
...
python -m wombat_transport.gc_harness write-pjc-input \
  validation_runs/cases/realistic_restart_noemis_2x25/wombat/main/run.yml tools/gc_harness/work/pjc_input.nc
...
python -m wombat_transport.gc_harness transport-step \
  validation_runs/cases/realistic_restart_noemis_2x25/wombat/main/run.yml --max-tracers 1
...
The registered large fixtures use versioned IDs. The current full transport
chain fixtures are `base_initial_transport_chain_v3`,
`base_initial_vdiff_after_tpcore_v3`, and
`base_initial_convection_fullgrid_v3`; keep those regenerated together when
handoff tests show pressure or tracer mismatches between cached stages.
...
It is generated from `validation_runs/cases/realistic_restart_noemis_2x25/wombat/main/run.yml`, the base restart, and local MERRA2
...
`validation_runs/cases/residual_24tracer_emissions_1day_2x25/wombat/main/run.yml`. It exercises the residual
...
`base_initial_transport_chain_v3` is the current full-grid one-tracer
...
`base_initial_vdiff_after_tpcore_v3` and
`base_initial_convection_fullgrid_v3` isolate the full-grid VDIFF and
...
python -m wombat_transport.gc_harness oracle-fixture-compare base_initial_transport_chain_v3

python -m wombat_transport.gc_harness oracle-fixture-handoff-compare base_initial_transport_chain_v3

python -m wombat_transport.gc_harness oracle-fixture-compare base_initial_vdiff_after_tpcore_v3

python -m wombat_transport.gc_harness oracle-fixture-compare base_initial_convection_fullgrid_v3
...
## Instrumented Full-Run/Main-Loop Trace

When full-run drift appears despite one-step operator parity, use the
main-loop trace tools to compare the high-level GEOS-Chem Classic workflow
against Wombat at the same boundary names. This is the tool to reach for when
operator fixtures pass but a real run drifts.

There are two levels of output:

- CSV/NetCDF column traces: selected columns, all levels, and selected tracers
  at named boundaries such as `before_tpcore_fvdas`, `after_do_vdiff`, and
  `after_do_convection`.
- Optional full-grid binary snapshots: full tracer fields at selected transport
  step occurrences for locating spatial patterns beyond the traced columns.
...
The generator writes instrumented copies of `main.F90`, `mixing_mod.F90`, and
`transport_mod.F90`, plus `wombat_main_loop_trace_mod.F90`. Build those into a
disposable GEOS-Chem Classic executable for the copied run directory. The trace
module supports these environment variables:

- `WOMBAT_GC_TRACE_CSV`: selected-column CSV trace output path.
- `WOMBAT_GC_SNAPSHOT_DIR`: optional directory for full-grid tracer snapshots.
- `WOMBAT_GC_SNAPSHOT_STEPS`: comma-separated 0-based boundary-occurrence
  indices to snapshot, usually `0` for the first transport step.
- `WOMBAT_GC_TPCORE_TRACE_NC`: optional TPCORE internal checkpoint NetCDF path.

Run the instrumented executable with the desired outputs, then compare the
selected-column traces:

```bash
WOMBAT_GC_TRACE_CSV=tools/gc_harness/work/gc_main_loop_trace.csv \
WOMBAT_GC_SNAPSHOT_DIR=tools/gc_harness/work/gc_main_loop_snapshots \
WOMBAT_GC_SNAPSHOT_STEPS=0 \
WOMBAT_GC_TPCORE_TRACE_NC=tools/gc_harness/work/gc_tpcore_trace.nc \
./gcclassic
...
Compare full-grid snapshots when enabled:

```bash
python tools/compare_main_loop_snapshots.py \
  tools/gc_harness/work/gc_main_loop_snapshots \
  tools/gc_harness/work/wombat_main_loop_snapshots \
  --step 0
```

The default traced columns are the first-day `SpeciesConc` hotspot columns
identified in the base no-emissions comparison. Pass repeated `--column LAT,LON`
values to `trace_wombat_main_loop.py` and matching repeated `--column-index I,J`
values to `generate_main_loop_trace_source.py` to trace different columns.
`trace_wombat_main_loop.py` uses Wombat lat/lon centers; the GEOS-Chem generator
uses 1-based GEOS-Chem `(I,J)` indices.

## Build Sketch

The build scripts link against
`validation_runs/work/realistic_restart_noemis_2x25/main/geoschem/build` by default.
...
```bash
python -m wombat_transport.gc_harness pjc-pfix validation_runs/cases/realistic_restart_noemis_2x25/wombat/main/run.yml
...
Current smoke result on `validation_runs/cases/realistic_restart_noemis_2x25/wombat/main/run.yml` after building against
`validation_runs/work/realistic_restart_noemis_2x25/main/geoschem/build`:
...
Current one-tracer transport-step smoke result on `validation_runs/cases/realistic_restart_noemis_2x25/wombat/main/run.yml`:
