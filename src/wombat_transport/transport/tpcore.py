...
try:  # Experimental acceleration path; the pure NumPy implementation remains the fallback.
...
def _advect_tracers(
    *,
    ...
    numba_x = _numba_tpcore_x_enabled()
    numba_y = _numba_tpcore_y_enabled()
    numba_prepass = _numba_tpcore_prepass_enabled()
    prepass_workspace = _make_tpcore_prepass_numba_workspace(nlat, nlon, _ntracer) if numba_prepass else None
    x_workspace = _make_xtp_numba_workspace(nlat, nlon, _ntracer) if numba_x else None
    y_workspace = _make_ytp_numba_workspace(nlat, nlon, _ntracer) if numba_y else None
    ...
    for level in range(nlev):
        ...
        if numba_prepass:
            if prepass_workspace is None:
                raise AssertionError("missing TPCORE Numba prepass workspace")
            qqu, qqv, adx, ady = prepass_workspace
            _calc_advec_cross_terms_batch_numba(
                q[level],
                ua[level],
                va[level],
                int(jn[level]),
            ...
            _xadv_dao2_batch_numba(qqv, ua[level], int(jn[level]), int(js[level]), adx)
            _yadv_dao2_batch_numba(qqu, va[level], ady)
        else:
        ...
        if numba_x:
            if x_workspace is None:
                raise AssertionError("missing X TPCORE Numba workspace")
            _xtp_batch_numba(
                dq1[level],
                qqv,
                pu[level],
        ...
        if numba_y:
            if y_workspace is None:
                raise AssertionError("missing Y TPCORE Numba workspace")
            _ytp_batch_numba(
                dq1[level],
                qqu,
                qqv,
    ...
    if _numba_tpcore_z_enabled():
        _fzppm_batch_numba(delp1, wz, dq1, q)
    else:
...
def _xtp_batch_numba(
    ...
    if not _NUMBA_AVAILABLE:
        _xtp_batch(dq1, qqv, pu, cx, xmass, jn, js)
        return
...
def _ytp_batch_numba(
    ...
    if not _NUMBA_AVAILABLE:
        _ytp_batch(dq1, qqu, qqv, cy, ymass, geofac, geofac_pc)
        return
...
def _numba_tpcore_x_enabled() -> bool:
    if not _NUMBA_AVAILABLE:
        return False
    return _numba_tpcore_mode() in {"all", "xyz", "xy", "x"}


def _numba_tpcore_y_enabled() -> bool:
    if not _NUMBA_AVAILABLE:
        return False
    return _numba_tpcore_mode() in {"all", "xyz", "xy", "y"}


def _numba_tpcore_prepass_enabled() -> bool:
    if not _NUMBA_AVAILABLE:
        return False
    return _numba_tpcore_mode() in {"all", "xyz", "xy"}
